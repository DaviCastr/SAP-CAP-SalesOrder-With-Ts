...
type SalesOrderItemPropsCreate = Omit<SalesOrderItemProps, 'id'>;
...
export class SalesOrderItemModel {
    ...
    public static create(props: SalesOrderItemPropsCreate): SalesOrderItemModel {
    ...
    public validateCreationPayload(params: CreationPayload): CreationPayloadValidationResult {
        ...
        if (product.stock === 0) {
            return {
                hasError: true,
                error: new Error(`Product ${product.name}(${product.id}) sem estoque disponível.`)
            };
        }
