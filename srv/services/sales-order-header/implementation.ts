...
export class SalesOrderHeaderServiceImplementation implements SalesOrderHeaderService {
    constructor(
    ...
    public async beforeCreate(params: SalesOrderHeader): Promise<CreationPayloadValidationResult> {
        const productsValidationResult = await this.validateProductOnCreation(params);

        if (productsValidationResult.hasError) {
            return {
                hasError: true,
                error: new Error('Anything product found')
            };
        }
        ...
        if (customerValidationResult.hasError) {
            return {
                hasError: true,
                error: new Error(`Customer ${header.customerId} not found.`)
            };
        }
    ...
    public async cloneSalesOrder(id: string, loggedUser: User): Promise<CreationPayloadValidationResult> {
        ...
        if (!header) {
            return {
                hasError: true,
                error: new Error('Sales Order not found!')
            };
        }
    ...
    private async validateCustomerOnCreation(
        ...
        if (!customer) {
            return {
                hasError: true,
                error: new Error(`Customer ${customer} not found.`)
            };
        }
    ...
    private async getProductsByIds(
        ...
        const productIds: string[] = params.items?.map((item) => item.product_id) as string[];
    ...
    private getSalesOrderLog(salesOrderHeader: SalesOrderHeaderModel, user: LoggedUserModel): SalesOrderLogModel {
        return SalesOrderLogModel.create({
            ...
            userData: user.toStringifyObject(),
            orderData: salesOrderHeader.toStringifyObject()
        });
    }
}
